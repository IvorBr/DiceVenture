...
impl Plugin for IslandPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(Update, (
            (player_enters_island, detect_objective).run_if(server_running),
            (spawn_island_player, detect_leave).in_set(IslandSet)
        ));
    }
}
...
fn setup_island(
    ...
    for x in 0..16 {
        for z in 0..16 {            
            ...
            map.add_entity_ivec3(IVec3::new(x, 0, z), Tile::new(TileType::Terrain, terrain_id));
        }
    }

    //setup leave tile
    let leave_id = commands.spawn((
        Mesh3d(meshes.add(Cuboid::new(1.0, 1.0, 1.0))),
        MeshMaterial3d(materials.add(StandardMaterial {
            base_color: Color::srgb_u8(100, 255, 100),
            ..Default::default()
    ...
    .id();
    map.add_entity_ivec3(IVec3::new(8, 0, 16), Tile::new(TileType::Terrain, leave_id));

    
    //add one enemy if were on the server
...
fn player_enters_island(
    ...
    for FromClient { client_id, event } in island_enter_event.read() {
        println!("Spawning player: {:?}", client_id);
        commands.spawn((
            Player(*client_id),
